...
class SMTPClient:
    ...
    def __init__(self, account=None, server=None, port=None, username=None, password=None, use_ssl=False):
        """
                    ...
                    for port in [465, 587, 25]:
                        ...
                        try:
                            if port == 465:
                                import ssl
                                context = ssl.create_default_context()
                                self.conn = smtplib.SMTP_SSL(self.server, port, context=context)
                                self.conn.timeout = 30
                                self.conn.ehlo()
                            else:
            ...
            try:
                ...
                if cc_addrs:
                    ...
                    for addr in cc_addrs:
                        if isinstance(addr, str) and ',' in addr:
                            # 如果字符串中包含逗号，按逗号分割
                            processed_cc.extend([a.strip() for a in addr.split(',') if a.strip()])
                        elif addr:
                ...
                if bcc_addrs:
                    ...
                    for addr in bcc_addrs:
                        if isinstance(addr, str) and ',' in addr:
                            # 如果字符串中包含逗号，按逗号分割
                            processed_bcc.extend([a.strip() for a in addr.split(',') if a.strip()])
                        elif addr:
                ...
                try:
                    ...
                    # 如果发送失败，尝试重新连接并再次发送
                    retry_count += 1
                    print(f"发送邮件失败，第 {retry_count} 次重试 (最大 {max_retries} 次): {e}")
                ...
                retry_count += 1
                print(f"发送邮件时发生错误，第 {retry_count} 次重试 (最大 {max_retries} 次): {e}")
