...
class TDLibManager:
    ...
    def get_source_library_path(
        self, platform_name: Optional[str] = None, arch: Optional[str] = None
    ) -> Path:
        """
    ...
    def get_development_library_paths(
        self, platform_name: Optional[str] = None, arch: Optional[str] = None
    ) -> Tuple[Path, Path]:
        """
        ...
        if platform_name == "darwin":
            ...
            raise ValueError(
                f"Development setup not supported for platform: {platform_name}"
            )
    ...
    def setup_development_libraries(self, force: bool = False) -> bool:
        ...
        if platform_name == "windows":
            ...
            logger.warning(
                "Please manually compile TDLib for Windows or use WSL/Docker for development."
            )
            return False
        ...
        try:
            ...
            if not source_path.exists():
                ...
                logger.error(
                    f"Please ensure TDLib library for {platform_name}_{arch} is available."
                )
                return False
            ...
            if not force and bot_path.exists() and user_path.exists():
                logger.info(
                    f"Development libraries already exist for {platform_name}_{arch}"
                )
                return True
            ...
            logger.info(
                f"Successfully set up development libraries for {platform_name}_{arch}"
            )
    ...
    def validate_library_setup(self) -> bool:
        ...
        try:
            ...
            # Additional validation: try to load the library to check dependencies
            if bot_exists and user_exists:
                validation_result = self._validate_library_dependencies(bot_path)
                if not validation_result:
                    logger.error("Library dependency validation failed")
                    return False
    ...
    def _validate_library_dependencies(self, library_path: str) -> bool:
        """
        Validate that the library can be loaded and has all required dependencies.

        Args:
            library_path: Path to the library file to validate
        ...
        try:
            ...
            # Try to load the library to check for missing dependencies
            lib = ctypes.CDLL(library_path)
            logger.debug(f"Successfully loaded library: {library_path}")
            return True
        except OSError as e:
            ...
            if "libc++.so" in error_msg or "libstdc++.so" in error_msg:
                logger.error(
                    "Missing C++ runtime libraries. Install libc++1 and libc++abi1 packages."
                )
                if self._is_container_environment():
                    logger.error(
                        "In Docker: RUN apt-get update && apt-get install -y libc++1 libc++abi1"
                    )
                else:
                    logger.error(
                        "On Ubuntu/Debian: sudo apt-get install libc++1 libc++abi1"
                    )
                    logger.error("On CentOS/RHEL: sudo yum install libcxx libcxxabi")
            elif "libssl" in error_msg:
            ...
            logger.error(f"Unexpected error validating library {library_path}: {e}")
            return False
