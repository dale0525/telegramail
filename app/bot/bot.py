...
from app.bot.handlers import (
    ...
    start_command, help_command, check_command, accounts_command,
    settings_callback, reply_command_handler,
    addaccount_command,
    get_compose_handler
)
from app.database.operations import (
    get_user_emails, get_email_account_by_id, 
    MessageOperations, delete_email
)
...
class TelegramMailBot:
    ...
    async def _owner_only_middleware(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """
        所有者权限检查中间件
    ...
    def _setup_handlers(self):
        ...
        from app.bot.handlers import (
            ...
            settings_callback, 
            handle_settings_callback,
            addaccount_command, handle_account_conversation,
            reply_command_handler, 
            get_compose_handler
        )
        
        # 基本命令处理器
        self.application.add_handler(CommandHandler("start", self._check_owner(start_command)))
        self.application.add_handler(CommandHandler("help", self._check_owner(help_command)))
        self.application.add_handler(CommandHandler("check", self._check_owner(check_command)))
        self.application.add_handler(CommandHandler("accounts", self._check_owner(accounts_command)))
        ...
        # 将原始处理器包装在检查函数中
        for state, handlers in compose_handler.states.items():
            wrapped_handlers = []
            for handler in handlers:
                if hasattr(handler, 'callback'):
                    handler.callback = self._check_owner(handler.callback)
                wrapped_handlers.append(handler)
            compose_handler.states[state] = wrapped_handlers
        
        # 包装入口点
        for i, entry_point in enumerate(compose_handler.entry_points):
            if hasattr(entry_point, 'callback'):
                entry_point.callback = self._check_owner(entry_point.callback)
        ...
        # 添加回复和转发命令处理器
        self.application.add_handler(CommandHandler("reply", self._check_owner(reply_command_handler)))
        
        # 账户操作回调
        self.application.add_handler(CallbackQueryHandler(self._check_owner(settings_callback), pattern="^settings_"))
        self.application.add_handler(CallbackQueryHandler(self._check_owner(settings_callback), pattern="^delete_account_"))
        self.application.add_handler(CallbackQueryHandler(self._check_owner(settings_callback), pattern="^confirm_delete_account_"))
        self.application.add_handler(CallbackQueryHandler(self._check_owner(settings_callback), pattern="^cancel_delete_account$"))
        
        # 新增专门的邮件操作回调处理器
        # 删除邮件
        from app.bot.handlers.email_delete import handle_delete_email
        self.application.add_handler(CallbackQueryHandler(
            self._check_owner(lambda u, c: handle_delete_email(u, c, int(u.callback_query.data.split("_")[2]))),
            pattern="^delete_email_"
        ))
        
        # 添加文本消息处理器 - 处理确认/取消删除邮件的文本响应
        from app.bot.handlers.email_delete import handle_delete_confirmation, handle_delete_cancellation
